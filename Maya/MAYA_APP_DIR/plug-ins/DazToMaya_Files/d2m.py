...
d2mLogo = os.path.expanduser("~/maya/plug-ins/DazToMaya_Files/d2m_logo.png")
serialPath = os.path.expanduser("~/maya/plug-ins/DazToMaya_Files/d2m_serial.xml")
imgIcon = os.path.expanduser("~/maya/plug-ins/DazToMaya_Files/iconhelp.png")
txtConf = os.path.expanduser("~/maya/plug-ins/DazToMaya_Files/d2m.cfg")
...
if pc == 0:
	serialPath = "/users/Shared/Autodesk/maya/plug-ins/DazToMaya_Files/d2m_serial.xml"
...
serialNumberList = (
"7D2M0DA5J2M0DA5Ji3z1M5",
"4452JDALO52JDALO4Uz1M9",
"8D2M1DAbU2M1DAbUYTz1M3",
"3D2M3DAd72M3DAd7iez1M7",
"fD2MaDAiZ2MaDAiZX0z1Mf",
"72M41DA3KM41DA3KIDz1M5",
"4D2c4DAJi2c4DAJie2z1M9",
"8D2ccDAJi2ccDAJib1zM13",
"3DdcdDAiedcdDAie1MzSA7",
"fD2MaDAJi2MaDAJi0Vz1Mf",
"7D2M0DAbX2M0DAbXC3z1M5",
"4Dfc4DAJifc4DAJie1zOM9",
"8D21cDA3J21cDA3Jibz1M3",
"3D2MdDA1G2MdDA1GFYz1M7",
"fD2MaDAbJ2MaDAbJi0z1Mf",
"d39bJDALO9bJDALO16z170")
...
config= ElementTree.parse(serialPath)
	...
	def getValuesFromXml(self, sceneMat, xmlMat):
		...
		if arnoldMethodNew == True: #------------------------------------------
			...
			if xmlValue != None and xmlValue != "":
				...
				cmds.setAttr(matName + "_ai" + ".specularColor", 1.0, 1.0, 1.0, type = 'double3' )
		...
		if arnoldMethodNew == False: #- OLD METHOD OLD ARNOLD -----------------------------------------
			for matName in xmlMatList:
				...
				xmlValue = self.getXmlMat(xmlMat, 'Metallicity_Map')
				if xmlValue != None and xmlValue != "":
					self.connectImageNode(xmlValue, '.outAlpha', sceneMat + "_ai" + '.Ks')
...
def btnSaveWithText():
	...
	saveFileResult = cmds.fileDialog2(fileFilter=multipleFilters, dialogStyle=2)
	...
	if saveFileResult != None:
		...
		for fileNode in textureFileNodes:
			...
			if imagePath != outFilename:
				...
				fileNode.setAttr('fileTextureName', "images/" + str(justFilename))  
...
class waitDialog(object):
	 def __init__(self):
		try:
			...
			pass
		self.WindowWait = pm.window(waitWindowName, toolbox=True, maximizeButton=False, minimizeButton=False, sizeable=False, title="DazToMaya", widthHeight=(343, 55)) 
...
class waitDialog2(object):
	 def __init__(self):
		try:
			...
			pass
		self.WindowWait2 = pm.window(waitWindowName2, toolbox=True, maximizeButton=False, minimizeButton=False, sizeable=False, title="DazToMaya", widthHeight=(343, 55)) 
		cmds.columnLayout( "columnName01", adjustableColumn = True)
		cmds.separator( height=20, style='in' )
...
def vrayEyeFix():
	try:
		...
		pass
	try:
		mel.eval('setAttr "EyeMoisture.specularColor" -type double3 0.279221 0.279221 0.279221')
	except:
		pass
	try:
	...
	try:
		...
		pass
	try:
		mel.eval('setAttr "Cornea.specularColor" -type double3 0.75974 0.75974 0.75974')
	except:
		pass
	try:
...
def importFbx():
	...
	if scaleMenuValue == "Automatic":
		mel.eval('FBXImportConvertUnitString cm') #FORCE cm Correct Unit....... CHELO
	if scaleMenuValue == "x1 (default)":
		mel.eval('FBXImportConvertUnitString cm') #FORCE cm Correct Unit....... CHELO
	if scaleMenuValue == "x10 (biger)":
		mel.eval('FBXImportConvertUnitString mm') #FORCE cm Correct Unit....... CHELO
	if scaleMenuValue == "x0.1 (small)":
		mel.eval('FBXImportConvertUnitString dm') #FORCE cm Correct Unit....... CHELO
	if scaleMenuValue == "x0.01 (smaller)":
		mel.eval('FBXImportConvertUnitString m') #FORCE cm Correct Unit....... CHELO
...
def cleanMorphs(figureWithMorphs):
	...
	for b in blendShapesList:
		...
		if (bFixed.find("__")>1):   
			...
			bFixed = bFixed.replace(caca[0]+"__","") #remove weird stuff from BlendShape Name   
		bFixed = bFixed.replace("headInner__","")
		bFixed = bFixed.replace("head_eCTRL","")
		bFixed = bFixed.replace("head__","") #remove weird stuff from BlendShape Name
		bFixed = bFixed.replace("head_","") #remove weird stuff from BlendShape Name
		bFixed = bFixed.replace("PHM","") #remove weird stuff from BlendShape Name
		bFixed = bFixed.replace("CTRL","") #remove weird stuff from BlendShape Name
		bFixed = bFixed.replace("QT1","") #remove weird stuff from BlendShape Name
		bFixed = bFixed.replace("Shape","") #remove weird stuff from BlendShape Name
		oldMorph = figureWithMorphs + "." + b
		try:        
			mel.eval('aliasAttr %s %s' %(bFixed,oldMorph)) #Rename Morphs (Blendshapes)
		except:
			pass
		i += 1
...
def slider_drag_callback(*args): 
	...
	humanMats = ("Skin","SkinFace","Face","Nipple","Head","Neck","Ears","Torso","Hips","Shoulders","Arms","Forearms","Nipples","Hands","Legs","Feet")
	...
	for o in skinMats:
		#STANDARD--------------------------------------------------------------------------------
		try:
			objAttrSpecWeight = skinMats[i] + ".specularColor" #Arnold - Specular Weight
			cmds.setAttr( objAttrSpecWeight, valorSpecWeight*2,valorSpecWeight*2,valorSpecWeight*2)
		except:
			pass
		try:
			objAttrSpecRough = skinMats[i] + ".cosinePower" #Arnold - Specular Roughness
		...
		try:
			...
			pass
		try:
			objAttrSpecRough = skinMats[i] + ".specularRoughness" #Arnold - Specular Roughness
		...
		#VRAY--------------------------------------------------------------------------------
		try:
			objAttrSpecWeight = skinMats[i] + ".reflectionColorAmount" #Arnold - Specular Weight
			cmds.setAttr( objAttrSpecWeight, valorSpecWeight)
		except:
			pass
		try:
			objAttrSpecRough = skinMats[i] + ".reflectionGlossiness" #Arnold - Specular Roughness
			cmds.setAttr( objAttrSpecRough, valorSpecRough)
		except:
			pass
		#----------------------------------------------------------------------------------------
...
def connectTex(image,material,input):
	...
	try:
		...
		if conn:
			...
			cmds.connectAttr(newPlacer+'.outUV',newFile+'.uvCoord', force=True)
			cmds.connectAttr(newPlacer+'.outUvFilterSize',newFile+'.uvFilterSize', force=True)
			for i in connections:
				cmds.connectAttr(newPlacer+'.'+i,newFile+'.'+i, force=True)
			# now connect the file texture output to the material input
			cmds.connectAttr(newFile+'.outColor',material+'.'+input,f=1, force=True)
...
def eyeLashFix():
	...
	try:
		mel.eval('disconnectAttr %s.outColor %s.color' %(conn[0], lashMaterial))
		mel.eval('setAttr "%s.color" -type double3 0.051 0.051 0.051' %lashMaterial)
	except:
		pass
...
def dazToIk():
	...
	toeBonesLeft = ("lBigToe","lSmallToe1","lSmallToe2","lSmallToe3","lSmallToe4")
	for f in toeBonesLeft:
		if f in jointsList:
			parentar(f,"lToe")
	
	toeBonesRight = ("rBigToe","rSmallToe1","rSmallToe2","rSmallToe3","rSmallToe4")
...
#//////////////////////////////////////////////////////////////////////////////
#// CLASS // CONVERT TO VRAY
#//////////////////////////////////////////////////////////////////////////////

def extraEyeFixes():
	try:
		mel.eval('disconnectAttr LoganEyesRef.outTransparency EyeReflection.transparency')
			...
			for m in mats:
				...
				if "Reflection" in mats[i]:
					try:
						...
						mel.eval('setAttr "%s.opacity" -type double3 0.0779221 0.0779221 0.0779221'%mats[i])
...
def gen8lagrimalFix():
	try:
		mel.eval('setAttr "ncl1.transparency" -type double3 0.993007 0.993007 0.993007')
		mel.eval('setAttr "ncl1.cosinePower" 44.48951')
		mel.eval('setAttr "ncl1.specularColor" -type double3 0.013986 0.013986 0.013986')
	except:
		pass
...
def autoIK():
	...
	if count > 10000000:
		...
		result = cmds.confirmDialog( title='DazToMaya: Not Valid Scene', message=errormsg, button=['Ok','View Info'], defaultButton='Yes', cancelButton='No', dismissString='No' )
...
def btnGoHelp():
	webbrowser.open("http://docs.daz3d.com/doku.php/public/read_me/index/71381/start")
...
try:
	...
	pass
windowDazMain = cmds.window(windowName, toolbox=True, maximizeButton=False, minimizeButton=True, sizeable=False, title="DazToMaya v1.5e", widthHeight=(343, 452)) 
cmds.columnLayout( "columnName01", adjustableColumn = True)
...
cmds.rowColumnLayout(numberOfColumns=4, columnWidth=[(1, 85), (2, 55),(3, 65) ], columnSpacing=[(1,1 ), (2,0)])
cmds.text( label='  Import from: ', align='left')
cmds.radioCollection()
rButton0 = cmds.radioButton( "radioImpAuto",label='Temp', select=True, align='left' )
...
cmds.setParent('..')
cmds.button( label='Auto-Import', width=343,height=50, c=lambda *args: autoImportDaz())
cmds.separator( height=20, style='in' )
cmds.text( label='  If importing from Temp folder, save to another location (!)', align='center')
cmds.separator( height=8, style='none')
cmds.button( label='Save Scene with Textures...', width=343,height=45, c=lambda *args: btnSaveWithText())
...
if "askToSaveSceneWithTextures=0" in cfgSettings:
	checkSave = cmds.checkBox( label="Show me a reminder after importing from Temp folder.", changeCommand=lambda *args: tempFun() ,value=0 )
else:
	checkSave = cmds.checkBox( label="Show me a reminder after importing from Temp folder.", changeCommand=lambda *args: tempFun() ,value=1 )
cmds.setParent('..')

cmds.separator( height=15, style='in' )
cmds.rowColumnLayout(numberOfColumns=3, columnWidth=[(1, 225), (2, 8),(3, 95) ], columnSpacing=[(1,6 ), (2,0)])
cmds.button( label='Convert Materials', width=43,height=25, c=lambda *args: btnConvert())
cmds.separator( height=8, style='none')
...
cmds.separator( height=5, style='none' )
cmds.floatSliderGrp('SpecWeight', label='Specular Weight', field=True, precision=2, value=0, minValue=0, maxValue=1, dc=slider_drag_callback) 
cmds.floatSliderGrp('SpecRough', label='Specular Roughness', field=True, precision=2, value=0, minValue=0, maxValue=1, dc=slider_drag_callback) 
cmds.separator( height=18, style='in' )

cmds.rowColumnLayout(numberOfColumns=4, columnWidth=[(1, 10),(2, 50),(3, 10),(4, 200)])
cmds.separator( height=5, style='none' )
...
def windowErrorNotImport():
	...
	def closeErrorWindow():
		try:
			...
			pass
	closeErrorWindow()
	windowErrorNotImport = cmds.window(windowName, toolbox=True, maximizeButton=False, minimizeButton=False, sizeable=False, title="DazToMaya: Oops! - Nothing to Import", widthHeight=(672, 480)) 
	d2mLogo = os.path.expanduser("~/maya/plug-ins/DazToMaya_Files/d2m_err_nf.png")
	...
	cmds.rowColumnLayout(numberOfColumns=4, columnWidth=[(1, 160), (2,150),(3,50)], columnSpacing=[(1,1 ), (2,0)])
	cmds.separator( height=5, style='none' )
	cmds.button( label='Get Daz Studio', width=150,height=40, c=lambda *args: gotoDaz())
	cmds.separator( height=5, style='none' )
	cmds.button( label='OK', width=150,height=40, c=lambda *args: closeErrorWindow())
...
def showWindowAskToSave():
	try:
		...
		pass
	windowAskToSave = cmds.window(askToSaveWindowName, toolbox=True, maximizeButton=False, minimizeButton=False, sizeable=False, title="DazToMaya", widthHeight=(400, 200)) 
	cmds.columnLayout( "columnName01", adjustableColumn = True)
	cmds.separator( height=10, style='none' )
	...
	col = [0.5, 0.5, 0.0]
	cmds.button( label='Save Scene with Textures...', bgc=col, command=lambda *args: btnSaveWithText(), height=40)
	cmds.separator( height=10, style='none' )
	...
	cmds.button( label='Ok', width=200,c=lambda *args: closeAskToSave(),height=20 )
...
def btnConvert():
	...
	if mats == None or len(mats) < 1:
		...
		result = cmds.confirmDialog( title='DazToMaya', message=errormsg, button=['Ok'], defaultButton='Yes', cancelButton='No', dismissString='No' )
	else:
		if matConv == "Arnold":
			try:
...
def getSerial():
	cmds.launch(web="http://www.daz3d.com")

def EnterSerialCheck():
	serialEntered = cmds.textField("txtFieldSerialEnter", query=True, text=True)
	serialEntered = serialEntered.replace(" ", "")

	sCheck = 0
	if "U2" in serialEntered: sCheck = sCheck +1 
	if "YT" in serialEntered: sCheck = sCheck +1 
	if "D" in serialEntered: sCheck = sCheck +1 
	
	if sCheck == 3 and serialEntered != "":
		serial = ET.Element("serialPath")
		ET.SubElement(serial, "d2m_serial", number=serialEntered)
		tree = ET.ElementTree(serial)
		tree.write(serialPath)
		try:
			cmds.deleteUI(serialWindowName)
			cmds.confirmDialog( title='DazToMaya', message="Successfully Register! Thank You.                                 ", button=['OK'])
...
def d2mstart():
	if "2014" in mayaversion or "2015" in mayaversion or "2016" in mayaversion or "2017" in mayaversion or "2018" in mayaversion or "2019" in mayaversion or "2020" in mayaversion:
		...
		if "D" in serialNumber: sCheck = sCheck +1 

		if sCheck == 3 and serialNumber != "":
			cmds.showWindow(windowDazMain)
